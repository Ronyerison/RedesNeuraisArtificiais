...
public class MultiLayerPerceptron {
	...
	protected List<CamadaIntermediaria> camadasIntermediarias;
	protected CamadaDeSaida camadaDeSaida;
	protected int quantEpocas;
	protected double erroMedio;
	protected StringBuffer strResult;
	...
	public void treinamento(Double[][] amostras, Double[][] esperado,
			double taxaDeAprendizado, double precisao) {
		...
		do {
			...
			for (int i = 0; i < amostras.length; i++) {
				...
				for (int j = 1; j < camadasIntermediarias.size(); j++) {
					camadasIntermediarias.get(j).combinarEntradas(
							camadasIntermediarias.get(j - 1).getVetorSaida());
					camadasIntermediarias.get(j).gerarSaidas();
				}
				camadaDeSaida.combinarEntradas(camadasIntermediarias.get(
						camadasIntermediarias.size() - 1).getVetorSaida());
				camadaDeSaida.gerarSaidas();
				camadaDeSaida.calcularGradiente(esperado[i]);

				camadaDeSaida.ajustarPesos(
						taxaDeAprendizado,
						camadasIntermediarias.get(
								camadasIntermediarias.size() - 1)
								.getVetorSaida());
	...
	private void imprimirCamadaDeSaida() {
		for (int i = 0; i < camadaDeSaida.getQuantNeuronios(); i++) {
			for (int j = 0; j < camadaDeSaida.getNeuronios().get(i).getPesos().length; j++) {
				System.out
						.print(camadaDeSaida.getNeuronios().get(i).getPesos()[j]
								+ " ");
			}
			System.out.println("\n");
		}
	...
	private void ajustarCamadasIntermediarias(Double[][] amostras,
		...
		camadasIntermediarias.get(camadasIntermediarias.size() - 1)
				.calcularGradiente(camadaDeSaida);
		for (int j = camadasIntermediarias.size() - 1; j > 0; j--) {
			camadasIntermediarias.get(j).ajustarPesos(taxaDeAprendizado,
					camadasIntermediarias.get(j - 1).getVetorSaida());
			camadasIntermediarias.get(j - 1).calcularGradiente(
					camadasIntermediarias.get(j));
		}
		camadasIntermediarias.get(0).ajustarPesos(taxaDeAprendizado,
				amostras[i]);
	}
	...
	public void inicializarPesos(int tam) {
		if (camadasIntermediarias.size() > 0) {
			...
			for (int i = 1; i < camadasIntermediarias.size(); i++) {
				camadasIntermediarias.get(i).gerarPesos(
						camadasIntermediarias.get(i - 1).getQuantNeuronios());
			}
	 ...
	 * @param camadaDeSaida
	 */
	public MultiLayerPerceptron(List<CamadaIntermediaria> camadasIntermediarias, CamadaDeSaida camadaDeSaida) {
	...
	/**
	 * @param camadasIntermediarias
	 *            the camadasIntermediarias to set
	 */
	...
	public void pesosCamada(Camada camada) {
		for (int i = 0; i < camada.getQuantNeuronios(); i++) {
			strResult.append("Peso Neuronio " + i + " "
					+ Arrays.toString(camada.getNeuronios().get(i).getPesos())
					+ "\n");
		}
	}
